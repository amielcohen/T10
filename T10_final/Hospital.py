...
last_ID = 0
workers = {} # a dict of workers where the key is their ID and the value is either at work or no
beds = {}
...
def test_if_in_fill_pattern(str):
    ...
    if list_of_str[0] == "fill" and list_of_str[2] == "in":
        return True
    return False
...
class SecretaryHomePage(tk.Frame):  # מזכירה רפואית
    ...
    def request_refill(self):
        ...
        submit_button = tk.Button(refill_request_window, text="Submit", font=("Arial", 15),
                                  command=lambda: self.submit_report(refill_text.get("1.0", 'end-1c')))
        submit_button.pack()
...
class ManagerHomePage(tk.Frame):  # מנהל
    ...
    def __init__(self, parent, controller):
        tk.Frame.__init__(self, parent)
        self.configure(bg="#4f558f")
        equipment = tk.Button(self, text="Viewing Equipment Orders", font=("Arial", 16), command=self.check_supp,
                              bg="light blue")
        ...
        self.show_all_workers = tk.Button(self, text="View available workers", bg="#119981", font=('Arial Bold', 15),
                                          command=self.view_all_workers)
    ...
    def view_daily_report(self):
        ...
        # Enter button
        enter_button = tk.Button(button_frame, text="Watch Report",
                                 command=lambda: self.display_report_window(id_entry.get()))
    ...
    def Delete_worker_report(self, worker_id):
        if messagebox.askyesno("Security question",
                               "By clicking on the Delete button you accept to remove the details"):
        ...
        if (self.edit_work_path_window_is_open == False):
            ...
            submit_button = tk.Button(newWindow, text="Send", bg="light blue", font=("Arial Bold", 20),
                                      command=lambda: [self.update_work_path(ID.get(), work_path.get("1.0", "end")),
                                                       ID.delete(0, END), work_path.delete('1.0', END)])
    ...
    def check_supp(self):
        ...
        disinfectant = tk.Label(master, text='disinfectant', bg=items_color['disinfectant'], font=("Arial", 15)).place(
            x=180, y=100)
...
class WorkerHomePage(tk.Frame):  # עובד ניקיון
    def join_out_work(self):
        ...
        if (self.present['text'] == "Enter Work"):
            self.present.configure(bg="green", text="Leave a shift")
            ...
            if self.daily_report == False:
                tk.messagebox.showinfo(title="report not enterd",
                                       message="Please fill your daily report before leaving")
            else:
    ...
    def __init__(self, parent, controller):
        ...
        button = tk.Button(self, text="Logout", font=("Arial", 15),
                           command=lambda: [self.clean(), logout(), controller.show_frame(Loginpage),
                                            self.daily_report_reset()])
        ...
        self.button3 = tk.Button(self, text='Clean Places Notifications', bg='#905fa2', font=('Arial Bold', 15),
                                 width=23,
                                 command=self.view_notifications_clean_place)
        self.button3.place(x=300, y=250)
        self.button4 = tk.Button(self, text='Notifications of Deficiencies', bg='#d48f7c', font=('Arial Bold', 15),
                                 width=23,
