...
last_ID = 0
workers = {}
beds = {} # bed status's can be "free and clean", "cleaning needed", "occupied"
for i in range(1,101):
    beds[i] = "free and clean"
...
class SecretaryHomePage(tk.Frame):  # מזכירה רפואית
    def __init__(self, parent, controller):
        ...
        self.change_bed_status_button = tk.Button(self, text ="Change a bed's status by key", font=("Arial", 15), command=self.change_bed_status)
    ...
    def request_refill(self):
            ...
            # Add checkbuttons for each supply
            self.soap_var = tk.IntVar()
            soap_checkbutton = tk.Checkbutton(refill_request_window, text="Soap", variable=self.soap_var)
            soap_checkbutton.pack()

            self.bleach_var = tk.IntVar()
            bleach_checkbutton = tk.Checkbutton(refill_request_window, text="Bleach", variable=self.bleach_var)
            bleach_checkbutton.pack()

            self.disinfectant_var = tk.IntVar()
            disinfectant_checkbutton = tk.Checkbutton(refill_request_window, text="Disinfectant",
                                                      variable=self.disinfectant_var)
            disinfectant_checkbutton.pack()

            self.broom_var = tk.IntVar()
            broom_checkbutton = tk.Checkbutton(refill_request_window, text="Broom", variable=self.broom_var)
            broom_checkbutton.pack()

            self.mop_var = tk.IntVar()
            mop_checkbutton = tk.Checkbutton(refill_request_window, text="Mop", variable=self.mop_var)
            mop_checkbutton.pack()

            self.rags_var = tk.IntVar()
            rags_checkbutton = tk.Checkbutton(refill_request_window, text="Rags", variable=self.rags_var)
            rags_checkbutton.pack()

            self.tissue_var = tk.IntVar()
            tissue_checkbutton = tk.Checkbutton(refill_request_window, text="Tissue", variable=self.tissue_var)
            tissue_checkbutton.pack()

            self.masks_var = tk.IntVar()
            masks_checkbutton = tk.Checkbutton(refill_request_window, text="Masks", variable=self.masks_var)
            masks_checkbutton.pack()

            self.gloves_var = tk.IntVar()
            gloves_checkbutton = tk.Checkbutton(refill_request_window, text="Gloves", variable=self.gloves_var)
            gloves_checkbutton.pack()
    ...
    def call_cleaner(self):
        ...
        submit_button = tk.Button(report_window, text="Submit", font=("Arial", 15),
                              command=lambda: self.submit_report(report_text.get("1.0", 'end-1c')))
        submit_button.pack()


    def submit_report(self, report):
        # Save the report to a text file
        with open("cleaner_report.txt", "w") as f:
           f.write(report)
        # Send a notification to the cleaner
        send_notification_to_cleaner()  # Replace with code to send notification
    ...
    def show_bed_status_by_serial_number(self, serial_number, bed_status_window):
        ...
        if bed_status is None:
            ...
            status_label = tk.Label(bed_status_window, text=f"Bed with serial number {serial_number} status is: {bed_status}.",
                                    font=("Arial", 15))
        status_label.pack()
    ...
    def change_bed_status(self):
        ...
        if int(serial_number) in beds:
            ...
            new_status = simpledialog.askstring("Change bed status",
                                                "Enter the new status for the bed (\"free and clean\", \"cleaning needed\", \"occupied\"):",
                                                parent=self)
            if new_status in ['free and clean', 'cleaning needed', 'occupied']:
